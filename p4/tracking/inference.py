    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        for p in self.legalPositions:
          ...
          for newPos, prob in newPosDist.items():
            allPossible[newPos] += prob * priors[p]
          
        #for p in self.legalPositions:
        #  allPossible[p] *= priors[p]
...
class ParticleFilter(InferenceModule):
    ...
    def getBeliefDistribution(self):
        ...
        injail = [x for x in self.particles if x == jpos]
        belief[self.getJailPosition()] =  (len(injail) * 1.0) / (self.numParticles * 1.0)
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        permCount = len(self.permutations)
        if(permCount > self.numParticles):
          shuffle(self.permutations)
          self.particles = self.permutations[0:self.numParticles]
