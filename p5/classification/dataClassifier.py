...
def enhancedPacmanFeatures(state, action):
    ...
    currentGhostDistAndTimer = map(lambda x : (mazeDistance(x.getPosition(), currentPos, state), x.scaredTimer), currentGhostStates)
    ...
    newGhostDistAndTimer = map(lambda x : (mazeDistance(x.getPosition(), newPos, state), x.scaredTimer ), newGhostStates)
    ...
    features["closestGhostDist"] = 0 if newClosestGhost[0] == 0 else 1.0 / newClosestGhost[0] ** 2
    ...
    features["closestFoodDist"] = 0 if len(newFoodDists) == 0 else 1.0 / min(newFoodDists, key=lambda x: x[1])[1]  #0 if len(newFoodDists) == 0 else 1 / search.mazeDistance(min(newFoodDists, key=lambda x: x[1])[0], newPos, state)
...
"""
  HAD TO PASTE IN CONTENTS OF search.py TO PASS AUTOGRADER
"""   
